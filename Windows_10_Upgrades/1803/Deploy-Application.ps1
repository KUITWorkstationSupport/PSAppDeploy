...
Try {
	...
	[string]$appVersion = '1803'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '05/07/2018'
	[string]$appScriptAuthor = 'Calvin Schulte'
	##*===============================================
	## Variables: Install Titles (Only set here to override defaults set by the toolkit)
	[string]$installName = 'Microsoft Windows 10 1803 Upgrade'
	[string]$installTitle = 'Microsoft Windows 10 1803 Upgrade'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		#Only start on AC Power
			$ACCheck = (Test-Battery -PassThru).IsUsingACPower
			If ($ACCheck -eq $True)
				{
					Show-InstallationWelcome -AllowDefer -DeferTimes 10 -CheckDiskSpace -RequiredDiskSpace 20000 -PersistPrompt -MinimizeWindows $False
				}
			Else
				{
					Write-Host "Stopping because not on AC Power." -ForegroundColor Red
					Exit-Script -ExitCode 69001
				}

		## Show Progress Message (with the default message)
			Show-InstallationProgress -StatusMessage 'Preparing to upgrade Windows...'
		...
		## <Perform Installation tasks here>
			...
			#Show-InstallationPrompt -Message 'In order to complete this Windows 10 upgrade, a restart is required. This restart could take up to 60 minutes.' -ButtonRightText 'Dismiss' -Icon Warning -NoWait
